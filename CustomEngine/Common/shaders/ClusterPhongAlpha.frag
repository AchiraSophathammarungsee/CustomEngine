...
void main()
{ 
  //if(material.diffuse.a < 0.5) discard;
  ...
  vec3 diffTex = material.diffuse.rgb;
  ...
  FragColor.a = material.diffuse.a;
}
...
vec3 CalcDirLight(int index, vec3 normal, vec3 viewDir)
{
  ...
  // combine results
  vec3 diffuse  = dirLights[index].color.rgb * diff * material.diffuse.rgb;
                    ...
                    vec3 viewDir, vec3 albedo)
{
    ...
    vec3 radiance = radianceIn * nDotL;// * ((material.diffuse.rgb / M_PI) + spec );
